...
exports.auth = (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.status(401).json({ success: false, error: error.message, message: 'Error while decoding token' });
    }
    next();
  } catch (error) {
    console.log('Error while token validating', error);
    return res.status(500).json({ success: false, message: 'Error while token validating' });
  }
}
...
exports.isStudent = (req, res, next) => {
  try {
    if (req.user?.accountType !== 'Student') {
      return res.status(403).json({ success: false, message: 'This Page is protected only for student' });
    }
    next();
  } catch (error) {
    console.log('Error while checking student role', error);
...
exports.isInstructor = (req, res, next) => {
  try {
    if (req.user?.accountType !== 'Instructor') {
      return res.status(403).json({ success: false, message: 'This Page is protected only for Instructor' });
    }
    next();
  } catch (error) {
    console.log('Error while checking instructor role', error);
...
exports.isAdmin = (req, res, next) => {
  try {
    if (req.user?.accountType !== 'Admin') {
      return res.status(403).json({ success: false, message: 'This Page is protected only for Admin' });
    }
    next();
  } catch (error) {
