...
exports.sendOTP = async (req, res) => {
  try {
    ...
    // Optionally show otp in response for dev/testing (do NOT enable in production)
    const showOtp = process.env.SHOW_OTP_IN_RESPONSE === 'true';
...
exports.signup = async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) {
      return res.status(400).json({ success: false, message: 'password & confirm password does not match' });
    }

    const checkUserAlreadyExists = await User.findOne({ email });
    if (checkUserAlreadyExists) {
      return res.status(400).json({ success: false, message: 'User registered already, go to Login Page' });
    }

    // find most recent otp stored for user
    const recentOtp = await OTP.findOne({ email }).sort({ createdAt: -1 });
    if (!recentOtp) {
      return res.status(400).json({ success: false, message: 'Otp not found in DB, please try again' });
    } else if (otp !== recentOtp.otp) {
    ...
    // fix approved logic: instructor should not be auto approved
...
exports.changePassword = async (req, res) => {
  try {
    ...
    console.log(error)
    res.status(500).json({ success: false, error: error.message, message: 'Error while changing password' })
  }
}
